...
def get_snotel_df(SiteName, years = 'recent'):
    ...
    threedaysago_string = (datetime.today() - timedelta(days = 3)).date().strftime('%Y-%m-%d')
    if years == 'recent':
...
for SiteName in ['Bear Lake']:
    ...
    min_time = noaa_df['startTime'].min()
    max_time = noaa_df['startTime'].max()

    temp_norm = plt.Normalize(22, 42)

    temp_plot = noaa_df.plot.scatter(x = 'startTime', y = 'temperature', ax = ax[0], xlim = (min_time, max_time), c = 'temperature', cmap = 'coolwarm', norm = temp_norm)
    ax[0].collections[0].colorbar.remove()
    ax[0].set_ylabel('Temperature (F)')
    snowfall_norm = plt.Normalize(0,0.5)
    noaa_df.plot.scatter(x = 'startTime', y = 'total_snowfall_in', ax = ax[1], xlim = (min_time, max_time), c = 'snowfall_in', norm = snowfall_norm, cmap = 'viridis')
    ax[1].collections[0].colorbar.remove()
    ax[1].set_ylabel('Snowfall (in)')

    ax11 = ax[1].twinx()
    noaa_df.plot(x = 'startTime', y = 'snowfall_in', ax = ax11, xlim = (min_time, max_time), c = tab10(1), x_compat=True, alpha = 0.3, legend = False, label = 'Snowfall Rate')
    # noaa_df.plot.bar(x = 'startTime', y = 'snowfall_in', ax = ax11, xlim = (min_time, max_time))
    ax11.set_ylabel('Snowfall Rate (in/hour)')
    ax11.fill_between(noaa_df['startTime'], noaa_df['snowfall_in'], 0, color=tab10(1), alpha=0.3)
    ...
    skycover_df.plot(x = 'startTime', y = 'value', ax = ax[2], label = 'Skycover', c = '#a3a3a3', x_compat = True, legend = False)
    ax[2].fill_between(skycover_df['startTime'], skycover_df['value'], 0, color='#a3a3a3', alpha=0.3)
    ax[2].fill_between(skycover_df['startTime'], skycover_df['value'], 100, color='#6bc9ff', alpha=0.3)
    ax[2].set_ylabel('Skycover (%)')

    wind_norm = plt.Normalize(0,30)
    idx = mpl.dates.date2num(noaa_df['startTime'])
    ...
    noaa_df_24h = noaa_df[noaa_df['startTime'].between(today,tomorrow)]
    ...
    email_password = 'bluzwkxzcxfzgnok'
    ...
    def send_emails(email_list):
        for person in email_list:
            ...
            msg['Subject'] = f'Daily Snow Report for {SiteName} - {date}'
